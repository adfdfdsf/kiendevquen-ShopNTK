...
public class ProductServices {
	...
	public List<ProductModel> searchProduct(String x) {
		...
		List<ProductModel> productModel = productEntity.stream().map(entity -> modelMapper.map(entity, ProductModel.class))
	...
	// search all
	public List<ProductModel> searchAllProduct() {
		List<ProductEntity> productEntity = productRepo.searchAllProduct();
		List<ProductModel> productModel = productEntity.stream().map(entity -> modelMapper.map(entity, ProductModel.class))
	            .collect(Collectors.toList());
		return productModel;
	}
