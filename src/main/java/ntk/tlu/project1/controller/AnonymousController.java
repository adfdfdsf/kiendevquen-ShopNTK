...
public class AnonymousController {
	...
	public String home(Model model) {
		...
		List<ProductModel> productModel2s = productServices.searchAllProduct();
...
//		// search product type(trang suc)
	...
	public String searchProduct(@RequestParam(value = "search", defaultValue = "") String search, Model model) {
		...
		List<ProductModel> productModels = productServices.searchProduct(search);
		model.addAttribute("productEntity", productModels);
	...
	public String detail(@RequestParam("id") int id, Model model) {
		ProductModel productModel = productServices.searchId(id);
